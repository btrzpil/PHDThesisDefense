...
\vspace{-1em}
    ...
    \column{.3\textwidth}
        \begin{figure}[H]
            \centering
            \includegraphics[scale=0.45]{ch1/lfp_ap_spectrum}  
            \end{figure}	
        \column{.3\textwidth}
        \begin{figure}[H]
            \centering
...
\begin{frame}[t]
    ...
    Podobnie kod źródłowy systemu akwizycji danych oparty o LabView oraz Python również nie jest dostępny publicznie.%, ponieważ konsekwentnie nie upubliczniałam w otwartych repozytoriach stworzonego systemu.
...
\begin{frame}[t]
    ...
    {\renewcommand\normalsize{\small}%
    \normalsize
    Eksperyment przeprowadzony w IBD był standardową procedurą opartą na doświadczeniu osoby go wykonującej, wykorzystującej różne systemy pomiarowe, dlatego w momencie opisu pracy uznano za wystarczające tego typu porównanie. 
    Dziękuję jednak za podanie propozycji, w jaki sposób można byłoby udoskonalić ten aspekt, co może się przydać podczas potencjalnej publikacji wyników układu scalonego.}
\end{frame}
...
\begin{frame}[t]
    ...
    {\renewcommand\normalsize{\small}%
        ...
        W pracy opisywane są wyniki, które doprowadziły do uzyskania zamierzonego celu, chociaż po drodze były różne koncepcje na temat tego, jak rozwiązać problem. 
        Zapewne to mogło również wpłynąć na to, że niektóre fragmenty są mniej zrozumiałe i lokalizacja wytłumaczenia może budzić pewne wątpliwości.
    }
\end{frame}
...
\begin{frame}[t]
    ...
    \begin{columns}
        ...
        {\renewcommand\normalsize{\small}%
            \normalsize
            \cit{Polarność sygnału wskazuje na kierunek przepływu jonów, do elektrody lub od elektrody.} - ten fragment tyczy się wytłumaczenia, dlaczego poszczególne panele się różną, nie odnoszą się do tego, co się stało na elektrodzie zero. 
            Zgodnie z wiedzą ekspercką osoby wykonującej pomiar i jej doświadczeniem w poprzednich eksperymentach efekt na elektrodzie zerowej został wytłumaczony jak efekt relaksacji mechanicznej tkanki.
            W pracy przedstawiono to wytłumaczenie jednak niestety w niezbyt klarowny sposób.

            \cit{Jeżeli uwzględnimy fakt, że pokazane dwie serie rejestracji zostały wykonane w znacznym odstępie czasowym ok. 100 minut, obserwację zmiany kształtu sygnału z bipolarnego na unipolarny należy interpretować jako zmianę w strukturze tkanki w pobliżu danej elektrody. 
